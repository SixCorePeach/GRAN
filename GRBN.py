...
class gcn2(nn.Module):
    ...
    def __init__(self, args, channel=32):
        ...
        self.rfb2_1 = RFB_modified(256, self.channel)
        self.rfb3_1 = RFB_modified(512, self.channel)
        self.rfb4_1 = RFB_modified(1024, self.channel)
        self.rfb5_1 = RFB_modified(2048, self.channel)
        ...
        self.mlp1 = nn.Conv2d(2 * self.channel, self.channel, 1)
        self.mlp2 = nn.Conv2d(2 * self.channel, self.channel, 1)
        self.mlp3 = nn.Conv2d(2 * self.channel, self.channel, 1)
        self.mlp4 = nn.Conv2d(2 * self.channel, self.channel, 1)

        self.mlp11 = nn.Conv2d(2 * self.channel, self.channel, 1)
        self.mlp22 = nn.Conv2d(2 * self.channel, self.channel, 1)
        self.mlp33 = nn.Conv2d(2 * self.channel, self.channel, 1)
        self.mlp44 = nn.Conv2d(2 * self.channel, self.channel, 1)
    ...
    def forward(self, x):
        ...
        seg_2_out = F.upsample(input=seg_2, size=(256, 256), mode='bilinear')
        #self.softmax()
        return seg_2_out
