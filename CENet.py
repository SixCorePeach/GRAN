...
class CE_Net_(nn.Module):
    def __init__(self, args, in_ch=3, num_classes = 2, feature_scale=4, is_deconv=True, is_batchnorm=True,
...
class up(nn.Module):
    def __init__(self, in_ch, out_ch, bilinear=True):
        ...
        if bilinear:
            self.up = F.interpolate(scale_factor=2, mode='bilinear', align_corners=False)
        else:
